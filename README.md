...
Catalogue de structures et d'habillages pour sites web.
**145 architectures** × **333 thèmes**, en HTML/CSS/JS pur, sans build ni dépendance.
...
**une architecture** et **un thème**. L'adaptation à son activité vient après, à la livraison.
...
- [Index transversal — par archétype](#index-transversal--par-archétype) — 31 archétypes de mise en page
- [Catalogue B — les thèmes](#catalogue-b--les-thèmes) — 333 thèmes en 13 familles
...
Deux axes indépendants, qui se croisent librement.

**L'architecture** définit la structure HTML : le type de barre de navigation, l'organisation
de la page principale, le découpage des sections, le comportement au défilement, la densité.

**Le thème** définit l'habillage : couleurs, typographie, rayons, ombres, épaisseurs,
courbes d'animation.

```
ARCHITECTURE (forme)      ×  THÈME (peau)       →  ce que voit le client
vitrine/03-sidebar-fixe   ×  dark-neon          →  dist/vitrine-03--dark-neon/
vitrine/03-sidebar-fixe   ×  earthy-terracotta  →  dist/vitrine-03--earthy-terracotta/
boutique/05-masonry       ×  dark-neon          →  dist/boutique-05--dark-neon/
```

Le HTML d'une architecture existe **une seule fois**. Ajouter un thème, c'est écrire
**un seul fichier CSS** — et il devient disponible sur les 145 architectures d'un coup.
C'est ce qui rend l'échelle du catalogue tenable.
...
Le client entre par celle qui lui parle.
...
| **Thématisation** | 100 % en CSS custom properties. Un thème = un fichier dans `_core/themes/`. |
| **Portée d'un thème** | Couleurs **et** typographie, rayons, ombres, épaisseurs de bordure, courbes d'animation. |
...
**Règle d'or :** aucune valeur de couleur en dur en dehors de `_core/themes/`.
Si le fichier de thème disparaît, la page doit rester lisible en noir et blanc sans casser.
...
│   ├── tokens.css          # déclaration + valeurs par défaut de toutes les variables
│   ├── blocks/             # blocs réutilisables — extraits au fil de la production
│   ├── stub.html           # page « à créer sur mesure » (optionnelle)
│   └── themes/             # 1 fichier CSS par thème, applicable partout
│       ├── modern-light.css
│       ├── dark-neon.css
│       └── neo-brutalism.css
├── _tools/
...
│   ├── check-contrast.js   # vérifie le contrat de contraste des thèmes  ✅
│   ├── new-theme.sh        # scaffolding d'un nouveau thème
│   ├── screenshot.js       # génère les preview.png
...
├── contact.html
├── assets/
│   ├── css/layout.css      # structure — aucune couleur en dur
│   ├── js/main.js
│   └── img/                # SVG neutres uniquement
├── meta.json               # famille, archétype, pages livrées, thèmes validés
├── preview.png
└── README.md               # 5 lignes : forme, pages livrées, où changer quoi
```
...
C'est le contrat qui permet à n'importe quel thème de s'appliquer à n'importe quelle
architecture. Toutes les variantes parlent le même vocabulaire de variables.
...
**Validation d'un nouveau thème :** l'appliquer sur `systeme/16-style-guide`. Tous les
rapports au vert, l'anneau de focus visible partout, aucun composant illisible.
...
Générer une architecture dans tous les thèmes d'un coup :

```bash
node _tools/build.js 12-systeme/16-style-guide --all-themes
```

Vérifier que les thèmes respectent le contrat de contraste :
...
---
# CATALOGUE B — LES THÈMES

13 familles. Un thème = **couleurs + typo + rayons + ombres + motion**, pas juste une teinte.
Chaque thème est un fichier `_core/themes/<slug>.css` applicable sur n'importe quel type.

Colonnes : `Dominantes` = les 2-4 couleurs pivot · `Typo` = couple titre/texte ·
`Signature` = ce qui rend le thème reconnaissable en 1 seconde.
...
les fondations ne soient éprouvées.
...
en trois thèmes. Quarante-cinq templates couvrant les demandes les plus courantes.

### Lot 3 — Une variante par archétype

Une variante pour chacun des trente et un archétypes, en un thème neutre.
Objectif : que l'index transversal soit intégralement navigable — un client peut demander
n'importe quelle forme et voir quelque chose.

### Lot 4 — Les thèmes spectaculaires

`synthwave` · `brutalist-type` · `geocities` · `terminal-green` · `art-deco` · `cyberpunk`
appliqués aux architectures déjà produites. Coût marginal (un fichier CSS), effet maximal
en démonstration.

### Lot 5 — Extraction des blocs

Une fois cinquante variantes produites, les blocs qui reviennent (barres de navigation, hero,
grilles, pieds de page, formulaires) sont extraits dans `_core/blocks/`. À partir de là,
composer une nouvelle architecture devient de l'assemblage.
...
- filtres croisés architecture × thème, et par tag technique
- sélecteur de thème en direct : changer l'habillage d'une variante sans recharger
- vue « comparer » : la même architecture sous quatre thèmes côte à côte
- vignettes générées automatiquement, recherche instantanée
- compteurs : *N architectures · M thèmes · K templates produits*
...
Une variante n'est considérée comme finie que quand tout est coché.

- [ ] Responsive vérifié à 320 / 768 / 1280 / 1920 px
- [ ] Aucune valeur de couleur en dur hors du fichier de thème
- [ ] Aucun mot orienté métier dans le contenu — libellés neutres et lorem ipsum uniquement
- [ ] Les liens vers les pages non livrées portent `data-stub`
- [ ] Les pages livrées annoncées dans `meta.json` existent réellement
- [ ] Navigation clavier complète, focus toujours visibles
- [ ] Contrastes AA minimum — AAA pour `a11y-first` et `high-contrast`
- [ ] `prefers-reduced-motion` respecté
- [ ] Aucune ressource externe (police, CDN, image distante)
- [ ] Formulaires : labels, `required`, messages d'erreur, état de succès
- [ ] Testée avec au moins trois thèmes très différents, dont un sombre et un brutaliste
- [ ] `preview.png` généré et `README.md` de variante rempli
